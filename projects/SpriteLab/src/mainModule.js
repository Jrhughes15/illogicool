...
import { initAnimationsPanel } from './ui/animations.js';
...
function wireProjectHeaderControls() {
  ...
  btnSave?.addEventListener('click', () => {
    try {
      ...
      setStatus && setStatus('Project saved.');
    } catch (err) {
      console.error('Failed to save project', err);
      setStatus && setStatus('Failed to save project.');
    }
  });

  btnImportProject?.addEventListener('click', () => fileInput?.click());
  btnExportProject?.addEventListener('click', () => btnSave?.click());
...
function wirePivotControls() {
  ...
  const refresh = () => {
    const key = getNearestPivotKey(getActiveFramePivot());
...
function serializeProject() {
  ...
  return {
    ...
    fps: App.fps,
    docTitle: docTitleEl?.textContent?.trim() || 'sprite_lab',
...
function loadProjectData(data) {
  ...
  if (data.docTitle && titleEl) titleEl.textContent = data.docTitle;
  applyFramesState(data.frames, data.activeFrameIndex ?? 0);
  renderPivotIndicator();
  renderOnionSkins();
